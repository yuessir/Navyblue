...
namespace NavyBule.Core.Util
{
    ...
    internal class CommonUtil
    {
        ...
        public static string GetFieldsEqPropertyName(IEnumerable<string> fieldList, string leftChar, string rightChar, string symbol = ":", Dictionary<string, string> fieldMap = null) //oracle @换成 
        {
            var fieldListInMap = new List<string>();
            if (fieldMap != null)
            {
                foreach (var item in fieldList)
                {
                    if (fieldMap.ContainsKey(item))
                    {
                        fieldListInMap.Add(item);
                    }
                }

            }
            StringBuilder sb = new StringBuilder();
            foreach (var item in fieldListInMap)
            {
                ...
                if (fieldMap != null)
                {
                    ...
                    sb.AppendFormat("{0}{1}{2}={4}{3}", leftChar, fieldMap[item], rightChar, item, symbol);
                }
                else
                {
                    sb.AppendFormat("{0}{1}{2}={3}{1}", leftChar, item, rightChar, symbol);
        ...
        public static TableEntity CreateTableEntity(Type t)
        {
            ...
            else
            {
                ...
                model.AllFieldPropertiesList = new List<string>();
                model.ExceptKeyFieldList = new List<string>();
                model.ExceptKeyAllFieldPropertiesList = new List<string>();
                model.ExceptKeyAllFieldPropertiesMap = new Dictionary<string, string>();
                model.AllFieldPropertiesMap = new Dictionary<string, string>();
                ...
                foreach (var item in allproperties)
                {
                    ...
                    if (igore == null)
                    {
                        ...
                        {
                            model.ExceptKeyAllFieldPropertiesList.Add(item.Name); //去除主键后的所有property列

                        }

                        model.AllFieldPropertiesMap.Add(item.Name, Name);
                        model.AllFieldList.Add(Name); //所有列

                        if (Name.ToLower().Equals(model.KeyName.ToLower()))
                        {
                            model.KeyType = item.PropertyType;
                            model.KeyPropertyName = item.Name;
                        }
                        else
        ...
        {
            string Fields = GetFieldsStr(table.AllFieldList, "", "");
            string FieldsAt = GetFieldsAtStr(table.AllFieldPropertiesList, ":");
            string FieldsEq = GetFieldsEqStr(table.AllFieldList, "\"", "\"", ":");
        ...
        {
            string updateList = GetFieldsEqStr(updateFields.Split(',').ToList(), leftChar, rightChar, symbol: symbol);
        ...
        public static string CreateUpdateSql(TableEntity table, string updateFields, string leftChar, string rightChar, Dictionary<string, string> fieldMap = null, string symbol = ":") //oracle @换成 :
        {
            string updateList = GetFieldsEqPropertyName(updateFields.Split(',').ToList(), leftChar, rightChar, symbol, fieldMap);
            return string.Format("UPDATE {0}{1}{2} SET {3} WHERE {0}{4}{2}={6}{5}", leftChar, table.TableName, rightChar, updateList, table.KeyName, table.KeyPropertyName, symbol);
        }
