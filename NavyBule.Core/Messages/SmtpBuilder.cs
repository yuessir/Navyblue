...
namespace Rhema.Core.Messages
{
    ...
    {
        #region Fields
        private readonly ILogger<SmtpBuilder> _logger;
        #endregion

        #region Ctor

        public SmtpBuilder(ILogger<SmtpBuilder> logger)
        {
            _logger = logger;
        ...
        {
            //var nopConfig = EngineContext.Current.Resolve<RMAConfig>();
            //emailAccount = nopConfig.EmailAccount;
            if (emailAccount is null)
            {
                _logger.LogError("Email account could not be loaded");
            }
